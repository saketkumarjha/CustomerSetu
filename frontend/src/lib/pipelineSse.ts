...
export function dispatchPipelineSseData(
  ...
  if (agentEvt) {
    sseLog(onLog, `agent_update ${agentEvt.agent}`, {
      status: agentEvt.status,
      order: agentEvt.order,
    });
  ...
  if (
    evt === "escalation_triggered" ||
    evt === "assigned_to_agent" ||
    evt === "response_sent"
  ) {
  ...
  // Tier completion events from escalation orchestrator
  if (
    raw.agent &&
    typeof raw.agent === "string" &&
    raw.agent.includes("Partial Pipeline")
  ) {
    sseLog(onLog, `tier_completion ${raw.agent}`, {
      tier: raw.tier,
      status: raw.status,
    });
    return;
 ...
 */
export function attachPipelineEventSource(
  es: EventSource,
  cbs: PipelineSseCallbacks,
): () => void {
  ...
  es.onerror = () => {
    sseLog(
      onLog,
      "EventSource onerror (connection lost, wrong URL, or 401 — check Network tab)",
    );
