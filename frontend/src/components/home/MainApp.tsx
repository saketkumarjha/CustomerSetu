...
import { CustomerFeedbackAnalytics } from "../analytics/CustomerFeedbackAnalytics";
import { RootCauseAnalytics } from "../analytics/RootCauseAnalytics";
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-100 via-slate-50 to-ub-blue-light/50 font-sans">
      ...
      >
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6 min-w-0">
          ...
          {active === "analytics-customer-feedback" && (
            <CustomerFeedbackAnalytics />
          )}
          {active === "analytics-root-cause" && <RootCauseAnalytics />}
          {active === "rbi" && <RBITab />}
          {active === "sla" && <SLATab />}
