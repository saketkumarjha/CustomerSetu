...
function IncidentAlertCard({ alert }: { alert: IncidentAlert }) {
  const cfg = PRIORITY_CONFIG[alert.alert_priority] ?? PRIORITY_CONFIG.low;
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        {alert.last_seen_at && (
          <span className="text-xs text-gray-400 flex-shrink-0 flex items-center gap-1">
            <Clock size={10} />
            {formatAgo(alert.last_seen_at)}
          </span>
        )}
      </div>

      {/* Product / region */}
      {(alert.product || alert.region) && (
        <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-gray-600 mb-2">
          {alert.product && (
            <span>
              Product:{" "}
              <strong>{alert.product.replace(/_/g, " ")}</strong>
            </span>
          )}
          {alert.region && (
            <span>
              Region: <strong>{alert.region}</strong>
            </span>
          )}
        </div>
      )}
      ...
      {alert.departments.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-3">
          {alert.departments.map((d) => (
            <span
              key={d}
              className="px-2 py-0.5 text-xs rounded-full bg-white border border-gray-200 text-gray-700 font-medium"
            >
              {d}
            </span>
          ))}
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
        {alert.risk_score != null && (
          <span className="flex items-center gap-1">
            <ShieldAlert size={11} />
            Risk{" "}
            <strong className={cfg.text}>{alert.risk_score}/100</strong>
          </span>
        )}
        {alert.growth_probability != null && (
          <span className="flex items-center gap-1">
            <TrendingUp size={11} />
            Growth{" "}
            <strong>{Math.round(alert.growth_probability * 100)}%</strong>
          </span>
        )}
        {alert.predicted_impact_customers != null && (
          <span>
            ~<strong>{alert.predicted_impact_customers}</strong> customers
            at risk
          </span>
        )}
        ...
        {(alert.rbi_reportable_ratio ?? 0) > 0 && (
          <span className="text-red-600 font-medium">RBI reportable</span>
        )}
      </div>
    </div>
  );
...
export function AgentDeskTab() {
  ...
  } = useApiData(() => api.incidents.list(), []);

  // Auto-refresh incident alerts every 60 seconds
  useEffect(() => {
    const id = setInterval(refetchAlerts, 60_000);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        ...
        <p className="text-xs text-gray-500 mb-4">
          Patterns detected by the ML model across all incoming complaints.
          Broadcast to all agents — act if the departments listed match your
          role.
        </p>
        {alertsLoading && (
          <div className="flex items-center gap-2 text-xs text-gray-400 py-4">
            <Loader2 size={14} className="animate-spin" /> Checking for
            alerts…
          </div>
        )}
        {alertsError && (
        ...
        {alertsData && !alertsLoading && alertsData.total === 0 && (
          <p className="text-xs text-gray-400 py-4 text-center">
            No active incidents detected. The system is watching.
          </p>
        )}
        {alertsData && alertsData.alerts.length > 0 && (
          <div className="space-y-3">
            {alertsData.alerts.map((alert) => (
              <IncidentAlertCard key={alert.cluster_id} alert={alert} />
            ))}
          </div>
        )}
      </div>
