...
**Incomplete Complaints Wasting AI Resources (Garbage-In Garbage-Out):** Solved via the **Customer Requirement Engine (CRE)** — a pre-pipeline gate that checks whether a complaint contains enough structured information (transaction ID, date, amount, account number) before allowing it to enter the expensive 6-agent AI pipeline. Incomplete complaints receive a temporary ID and a guided gap-fill flow instead of being silently mis-processed.

---

## 🛡️ Customer Requirement Engine (CRE)

> **The smart front door of CustomerSetu.** Before a complaint reaches any AI agent, CRE decides whether it has enough information to actually be solved.

### Why CRE Exists

The most common cause of AI hallucination and wrong resolutions is not a bad model — it is a bad complaint. When a customer writes *"my money is gone please help"*, the AI has no transaction ID, no date, no amount. It will generate a plausible-sounding but factually empty response. CRE catches this at the door.

At scale (1,000,000 complaints/day), routing incomplete complaints through 6 GPT-4o agents wastes compute, increases latency, and produces resolutions that agents have to manually correct. CRE eliminates this class of problem entirely.

### CRE Decision Tree — L0 → L1 → L2

```mermaid
flowchart TD
  IN[Customer submits complaint] --> CACHE{Result cached?\nSHA-256 hash}
  CACHE -->|Hit — 0ms| OUT[Return cached result]
  CACHE -->|Miss| L0{L0 — Keyword Gate\n0.1ms · No AI}

  L0 -->|Text under 20 chars| REJ[REJECTED\nAsk customer to describe issue]
  L0 -->|Pure greeting — hi hello namaste| AWA[AWAITING INFO\nAsk for banking issue]
  L0 -->|General query — balance ifsc interest| ADE0[ADEQUATE\nGeneral enquiry · pass to pipeline]
  L0 -->|Banking problem keyword found| PRIO[L0-B Priority Scorer\nfraud=5 · upi=4 · loan=3 · general=1]

  PRIO --> L1{L1 — Regex Extraction\n2ms · No AI}
  L1 --> CAT[Detect category from weighted keywords\nUPI·ATM·Card·Loan·FD·Fraud etc]
  CAT --> EXT[Extract fields via regex\ntxn_id · txn_date · amount\ncif_or_account · card_last4 · contact]
  EXT --> CHK{All required fields\nfor this category present?}
  CHK -->|Yes| ADE1[ADEQUATE\nPass to pipeline with priority_score]
  CHK -->|No| L2GATE{CRE_L2_ENABLED\nenv flag}
  L2GATE -->|false — default| TMPCREATE[Create TMP Draft\nStore in complaint_drafts]
  L2GATE -->|true| L2[L2 — GPT-4o-mini\nFree-text field extraction\n+ friendly gap message]
  L2 --> CHK2{Now adequate?}
  CHK2 -->|Yes| ADE1
  CHK2 -->|No| TMPCREATE

  TMPCREATE --> LINK[Send gap-fill link to customer\n/draft/TMP-XXXXXXXX]
  LINK --> FILL[Customer fills missing info]
  FILL --> PATCH[Re-run CRE on combined text\nMerge extracted fields]
  PATCH --> PROMOTE{Adequate now?}
  PROMOTE -->|Yes| CMP[Promote to CMP-XXXXXXXX\nInsert into complaints table\nTrigger pipeline]
  PROMOTE -->|No| FILL
```

### The Three Tiers

| Tier | Technology | Latency | When It Runs | What It Does |
|------|-----------|---------|--------------|--------------|
| **L0 — Keyword Gate** | Pure Python, `master.json` rules | 0.1 ms | Always | Rejects nonsense, handles greetings, flags banking keywords, scores complaint priority (1–5) |
| **L1 — Regex Extraction** | Compiled regex patterns, `master.json` | 2 ms | When L0 passes to L1 | Detects complaint category (UPI/ATM/Loan/etc.), extracts transaction fields, determines exactly what is missing |
| **L2 — GPT-4o-mini** | OpenAI API call | ~4 s | Only when `CRE_L2_ENABLED=true` AND L1 has missing fields | Understands free-form text that regex misses ("I lost five thousand rupees on the fifteenth"), composes a friendly gap message |

### Temporary ID Flow — TMP → CMP

When a complaint is inadequate, CRE does not reject it. It creates a **draft** with a temporary ID and gives the customer a guided path to complete it:

```
1. Complaint arrives: "UPI money not received please help"
   → L1 detects category: UPI/IMPS/NEFT
   → L1 extracts: description ✓ — but missing: txn_id, txn_date, amount, account_number
   → CRE creates draft: TMP-A3F9C2D1 in complaint_drafts table
   → Customer receives: /draft/TMP-A3F9C2D1

2. Customer opens gap-fill page
   → Sees exactly what is missing ("Transaction ID", "Date of transaction", "Amount")
   → Submits: "UTR 123456789012, 15 June 2026, Rs 5000, account 9876543210"

3. CRE re-evaluates merged text
   → All required fields now present → ADEQUATE
   → Promotes to CMP-B7E2F1A3
   → Inserts into complaints table
   → Triggers full AI pipeline with priority_score from L0-B
```

### Priority Scoring — Fraud Jumps the Queue

L0-B assigns a `priority_score` to every complaint before it enters the pipeline queue. Fraud complaints are never waiting behind balance enquiries:

| Complaint Type | Priority Score | Examples |
|---|---|---|
| Fraud / Cyber Crime | **5 — Urgent** | `unauthorized transaction`, `phishing`, `without my knowledge` |
| Transaction Failure | **4 — High** | `debited`, `atm cash not dispensed`, `chargeback` |
| Loan / Cheque | **3 — Medium** | `emi`, `cheque bounce`, `loan foreclosure` |
| Account / Card | **2 — Normal** | `card blocked`, `account frozen` |
| General Enquiry | **1 — Low** | `balance`, `ifsc`, `branch timing` |

### Configuration — Off by Default, Production Safe

All CRE behaviour is controlled entirely by environment variables. The default is all flags off — the existing complaint pipeline is completely unaffected unless you explicitly opt in:

| Environment Variable | Default | Effect |
|---|---|---|
| `CRE_ENABLED` | `false` | Master switch. Off = CRE never runs, all complaints go directly to pipeline |
| `CRE_BLOCK_PIPELINE_IF_INADEQUATE` | `false` | Hard gate mode. On = pipeline refuses inadequate complaints with HTTP 409 |
| `CRE_L2_ENABLED` | `false` | Enables GPT-4o-mini tier for complex free-form extraction |
| `CRE_DRAFT_EXPIRY_DAYS` | `7` | TMP drafts expire after this many days if never completed |

### CRE API Endpoints

| Method | Endpoint | Purpose |
|---|---|---|
| `POST` | `/api/v1/cre/evaluate` | Stateless check — returns adequacy result without creating a draft |
| `POST` | `/api/v1/cre/intake` | Stateful intake — creates TMP draft if inadequate, returns TMP-xxx ID |
| `GET` | `/api/v1/cre/draft/{tmp_id}` | Fetch a draft and its current missing fields |
| `PATCH` | `/api/v1/cre/draft/{tmp_id}` | Customer submits gap-fill text — re-evaluates and updates draft |
| `POST` | `/api/v1/cre/draft/{tmp_id}/promote` | Promote adequate draft to full CMP-xxx complaint and trigger pipeline |

### CRE Database Table

CRE uses a completely separate `complaint_drafts` table. The existing `complaints` table is never modified by CRE:

```sql
-- Run migration: backend/migrations/013_cre_drafts.sql
CREATE TABLE IF NOT EXISTS complaint_drafts (
    tmp_id               TEXT        PRIMARY KEY,          -- e.g. TMP-A3F9C2D1
    channel              TEXT        NOT NULL DEFAULT 'web',
    customer_id          TEXT        NOT NULL DEFAULT '',
    original_text        TEXT        NOT NULL,
    merged_text          TEXT        NOT NULL,             -- grows with each gap-fill turn
    cre_status           TEXT        NOT NULL DEFAULT 'received',  -- received | awaiting_info | adequate | promoted | expired
    category             TEXT,                             -- detected complaint category
    tier_used            TEXT,                             -- L0 / L1 / L2
    extracted_fields     JSONB       NOT NULL DEFAULT '{}',
    field_confidence     JSONB       NOT NULL DEFAULT '{}',
    missing_fields       JSONB       NOT NULL DEFAULT '[]',
    gap_message          TEXT,                             -- friendly message shown to customer
    promoted_complaint_id TEXT,                            -- set when TMP → CMP promotion completes
    turn_count           INTEGER     NOT NULL DEFAULT 0,   -- number of gap-fill attempts
...
### What master.json Controls

All CRE rules are driven by a single configuration file (`backend/app/services/cre/master.json`) — no code changes needed to add new keywords, categories, or field requirements:

- **`l0.escalate_keywords`** — 65+ banking keywords in English and Hinglish that trigger L1 evaluation (`debited`, `upi`, `fraud`, `paisa kat gaya`, `refund chahiye`)
- **`l0.allow_keywords`** — General enquiry terms that pass as adequate without transaction fields (`balance`, `ifsc`, `interest rate`)
- **`l1.category_bundles`** — Weighted keyword sets for each complaint category (primary = 2 pts, secondary = 1 pt) covering UPI, ATM, Cards, Loans, FD, Fraud, KYC, Internet Banking and more
- **`l1.category_requirements`** — Per-category list of required fields (e.g., UPI complaints need `txn_id`, `txn_date`, `amount`, `cif_or_account`)
- **`l1.regex_extractors`** — Regex patterns for extracting each field from free text (UTR numbers, DD/MM/YYYY dates, ₹ amounts, 10-digit account numbers, card last-4 digits, phone, email)
...
graph TB
    ...
    subgraph CRE["🛡️ Customer Requirement Engine — Pre-Pipeline Gate"]
        direction TB
        CR0["L0 — Keyword Gate · 0.1ms · No AI\nReject junk · Score priority 1-5"]
        CR1["L1 — Regex Extraction · 2ms · No AI\nDetect category · Extract transaction fields"]
        CR2["L2 — GPT-4o-mini · opt-in · off by default\nFree-text extraction + gap message"]
        CR3["TMP Draft Store\ncomplaint_drafts table\nExpires in 7 days"]
        GAPFILL["🔗 Gap-Fill Page\n/draft/TMP-XXXXXXXX\nCustomer fills missing info"]
        CR0 --> CR1
        CR1 -- "Fields missing" --> CR2
        CR2 -- "Still incomplete" --> CR3
        CR3 --> GAPFILL
        GAPFILL -- "Now adequate → promote TMP→CMP" --> CR0
    end
    ...
    INTAKE --> CRE
    CRE -- "Adequate — with priority_score" --> PIPELINE
    CRE -- "Incomplete → TMP draft" --> STORAGE
    B1 --> B2
    B2 -- "Unique" --> B3
...
sequenceDiagram
    ...
    GW->>CRE: Evaluate adequacy — L0 keyword gate (0.1ms)
    alt Junk or greeting
        CRE-->>C: "Please describe your banking issue"
    else Incomplete — missing required fields
        CRE->>DB: Create TMP-XXXXXXXX draft in complaint_drafts
        CRE-->>C: "Please provide: Transaction ID, Date, Amount"
        C->>GW: Gap-fill submission at /draft/TMP-XXXXXXXX
        GW->>CRE: Re-evaluate merged text — L1 regex (2ms)
        CRE->>DB: Promote TMP → CMP-XXXXXXXX in complaints table
        Note over CRE,DB: Now adequate — enter pipeline with priority_score
    else Adequate complaint
        Note over CRE: L1 regex confirms all required fields present
    end
    CRE->>PII: Adequate complaint text + priority_score
...
- **L0 keyword gate** (0.1ms): rejects junk, handles greetings, scores complaint priority 1–5
- **L1 regex extraction** (2ms): detects category (UPI/ATM/Loan/Card/FD/Fraud), extracts transaction fields, lists exactly what is missing
- **L2 GPT-4o-mini** (opt-in, off by default): understands free-form amounts/dates that regex misses
- **TMP Draft system**: incomplete complaints get a `TMP-XXXXXXXX` ID and a gap-fill link — never lost, never rejected
- **Auto-promotion**: TMP → `CMP-XXXXXXXX` the moment adequacy is confirmed, pipeline triggers automatically
- **Priority queue**: fraud complaints (score 5) always processed before balance enquiries (score 1)
- **Zero production risk**: all three feature flags default to `false` — existing pipeline untouched
...
# Customer Requirement Engine (CRE) — all off by default, production safe
# Flip CRE_ENABLED=true to activate the pre-pipeline adequacy gate
CRE_ENABLED=false
CRE_BLOCK_PIPELINE_IF_INADEQUATE=false
CRE_L2_ENABLED=false
CRE_DRAFT_EXPIRY_DAYS=7
```
...
- `complaint_drafts` — **CRE**: temporary drafts (TMP-xxx) for incomplete complaints pending gap-fill (migration: `013_cre_drafts.sql`)
...
**CRE migration** — run this in the Supabase SQL editor to enable TMP draft storage (only needed if `CRE_ENABLED=true`):

```sql
-- backend/migrations/013_cre_drafts.sql
CREATE TABLE IF NOT EXISTS complaint_drafts (
    ...
    image_url            TEXT,
    cre_status           TEXT        NOT NULL DEFAULT 'received',
    category             TEXT,
    tier_used            TEXT,
    extracted_fields     JSONB       NOT NULL DEFAULT '{}',
    field_confidence     JSONB       NOT NULL DEFAULT '{}',
    missing_fields       JSONB       NOT NULL DEFAULT '[]',
    missing_field_labels JSONB       NOT NULL DEFAULT '[]',
    gap_message          TEXT,
    promoted_complaint_id TEXT,
    turn_count           INTEGER     NOT NULL DEFAULT 0,
    expires_at           TIMESTAMPTZ NOT NULL DEFAULT (NOW() + INTERVAL '7 days'),
    created_at           TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at           TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_drafts_customer_id ON complaint_drafts (customer_id);
CREATE INDEX IF NOT EXISTS idx_drafts_cre_status  ON complaint_drafts (cre_status);
CREATE INDEX IF NOT EXISTS idx_drafts_expires_at  ON complaint_drafts (expires_at);
```
...
│   │   │   ├── cre.py                     ← CRE: evaluate, intake, draft CRUD, TMP→CMP promote
...
│   │   │   │
│   │   │   ├── cre/                       ← Customer Requirement Engine (pre-pipeline gate)
│   │   │   │   ├── master.json            ← All CRE rules: keywords, categories, regex, requirements
│   │   │   │   ├── orchestrator.py        ← L0 → L1 → L2 pipeline + LRU cache + evaluate_cre()
│   │   │   │   ├── config_loader.py       ← Loads and caches master.json at startup
│   │   │   │   ├── draft_service.py       ← CRUD for complaint_drafts table (TMP lifecycle)
│   │   │   │   └── tiers/
│   │   │   │       ├── l0_rules.py        ← L0: keyword gate, greeting block, priority scorer
│   │   │   │       ├── l1_regex.py        ← L1: category detection + regex field extraction
│   │   │   │       └── l2_mini_llm.py     ← L2: GPT-4o-mini free-text extraction (opt-in)
│   │   │   │
│   │   │   ├── rbi/
...
│       ├── components/
│       │   ├── cre/
│       │   │   └── DraftGapFillPage.tsx   ← Gap-fill UI for TMP-XXXXXXXX drafts (missing field form)
│       │   └── ...                        ← (other component folders unchanged)
│       ├── lib/
│       │   ├── api.ts                     ← All API client calls (typed) — includes CRE methods
│       │   ├── cre.ts                     ← checkCreAdequacy() utility used by complaint forms
...
| **CRE off by default**         | The Customer Requirement Engine is disabled in all deployments (`CRE_ENABLED=false`). Enabling it requires running migration `013_cre_drafts.sql` first and setting the env flags explicitly.                       |
| **CRE L2 cost**                | When `CRE_L2_ENABLED=true`, every complaint that L1 cannot resolve makes a GPT-4o-mini call (~$0.0001 each). At 1M/day with a 30% L2 rate, budget ~$30/day for L2 alone.                                          |
...
| CRE L0 latency                   | **< 0.5 ms** (pure Python keyword match)          |
| CRE L1 latency                   | **< 3 ms** (compiled regex, no AI)                |
| CRE L2 latency                   | **~4 s** (GPT-4o-mini, opt-in only)               |
| CRE min field confidence         | **0.80** (below this, field treated as missing)   |
| CRE TMP draft expiry             | **7 days** (configurable via env)                 |
| CRE priority scores              | **1 (low) → 5 (fraud, urgent)**                   |
